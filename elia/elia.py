...
class EliaPandasClient:
    ...
    def get_solar_power_estimation_and_forecast(self, region: str = None, **params) -> pd.DataFrame:
    ...
    def get_wind_power_estimation_and_forecast(self, region: str = None, **params) -> pd.DataFrame:
    ...
    def _construct_where_filter(**kwargs) -> str:
        ...
        date_filter = f"datetime IN [date'{start.strftime(DATETIME_FORMAT)}'..date'{end.strftime(DATETIME_FORMAT)}'[" if (start and end) else None
